...
public class OrderDataRepository {
	...
	public List<OrderProductData> getOrderByClusterId(List<Integer> clusterIds, long hubId) {
		...
		String sql = " select ord.id as orderId,ord.id,ord.hub_id,ord.cluster_id,ord.customer_id,ord.society_id,ord.tower_id,"
				+ " op.product_id,op.quantity,op.category_id,op.size,op.price,op.sheet,op.packing_type,op.scanner_code,"
				+ " op.image,op.name,op.type,op.subcategory_id,op.description" + " from orders ord "
				+ " inner join order_product op on ord.id =op.order_id" + " where ord.hub_id=" + hubId
				+ " and ord.cluster_id in (" + String.join(",", cIds) + ")";
