...
import com.mb.demo.constants.PickPackConstants.OrderActionStatus;
...
import com.mb.demo.entity.common.ProductOrderMappingEntity;
...
public class CustomerOrderManagementRepository {
	...
	public CustomerOrderBean createOrder(OrderProductUpdateBean orderProductBean) {
		ProductEntity productEntity = updateProductInventory(orderProductBean.getProductId(),
		...
		if (null != productEntity) {
			...
			orderProductBean.setActionType(OrderActionStatus.ADD);
	...
	public ProductEntity updateOrder(OrderProductUpdateBean orderProductBean) {
		ProductEntity productEntity = updateProductInventory(orderProductBean.getProductId(),
				customerProfile.getHubId(), orderProductBean.getActionType());
		if (null != productEntity) {
			if (orderProductBean.getActionType() == OrderActionStatus.ADD && orderProductBean.getProductId() == 1) {
	...
	public void updateOrderProductMapping(OrderProductUpdateBean orderProductBean) {
		ProductOrderMappingEntity productOrderMapping = ProductOrderMappingEntity.builder()
				.customerId(customerProfile.getCustomerId()).productId(orderProductBean.getProductId())
				.quantity(orderProductBean.getQuantity()).build();

		productOrderMapping.setOrderId(orderProductBean.getOrderId());

		productOrderMapping = orderFactory.getOrderProductMapping().save(productOrderMapping);
	}

	public ProductEntity updateProductInventory(long productId, long hubId, OrderActionStatus operationType) {
		ProductEntity productEntity = getProductByIdAndHubId(productId, hubId);
	...
	public ProductEntity getProductByIdAndHubId(long productId, long hubId) {
		ProductEntity productEntity = orderFactory.getProductRepository().findByIdAndHubId(productId,
				customerProfile.getHubId());
		return productEntity;
	}

	public void addOrderProduct(ProductEntity productEntity, long orderId) {
